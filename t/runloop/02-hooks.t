...
$engine->add_hook(hook1 => "Kvasir::Hook::Perl" => sub { ok(++$i == 1) });
$engine->add_pre_hook("hook1");
$engine->add_hook(hook2 => "Kvasir::Hook::Perl" => sub { ok(++$i == 2) });
$engine->add_pre_hook("hook2");
my $cb = Kvasir::Runloop::_mk_runloop($engine);
$cb->();

$engine = Kvasir::Engine->new();
$engine->add_hook(hook1 => "Kvasir::Hook::Perl" => sub { ok(++$i == 3) });
$engine->add_post_hook("hook1");
$engine->add_hook(hook2 => "Kvasir::Hook::Perl" => sub { ok(++$i == 4) });
